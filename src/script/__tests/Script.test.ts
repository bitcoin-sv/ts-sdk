...
describe('Script', () => {
    ...
    describe('#fromBinary', () => {
        ...
        const script = new Script().fromBinary(buf);
        expect(script.chunks).toHaveLength(1);
    ...
    expect(() => {
      Script.fromASM(a[0]).toString();
      Script.fromASM(a[0]).toASM();
    }).not.toThrow();

    expect(() => {
      Script.fromASM(a[1]).toString();
    ...
    it(`should not fail when reading scriptInvalid vector ${i}`, () => {
      ...
      let strA = Script.fromASM(a[0]).toASM();
      ...
      let strB = Script.fromASM(a[1]).toASM();
