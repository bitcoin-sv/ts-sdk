...
import KeyDeriver, { Counterparty } from '../../wallet/KeyDeriver'
...
describe('KeyDeriver', () => {
  ...
  test('should compute the correct invoice number', () => {
    const invoiceNumber: string = keyDeriver.computeInvoiceNumber(
      protocolID,
      keyID
    )
  ...
  test('should throw if no counterparty given', () => {
    expect(() => keyDeriver.normalizeCounterparty(undefined as unknown as Counterparty)).toThrow()
  })

  test('should normalize counterparty correctly for self', () => {
    const normalized: PublicKey = keyDeriver.normalizeCounterparty('self')
    expect(normalized.toString()).toBe(rootPublicKey.toString())
  })

  test('should normalize counterparty correctly for anyone', () => {
    const normalized: PublicKey = keyDeriver.normalizeCounterparty('anyone')
  ...
  test('should normalize counterparty correctly when given as a hex string', () => {
    const normalized: PublicKey = keyDeriver.normalizeCounterparty(
  ...
  test('should reveal the specific key association', () => {
    const specificSecret: number[] = keyDeriver.revealSpecificSecret(
      counterpartyPublicKey,
      protocolID,
      keyID
    )
    ...
    const sharedSecretPoint = rootPrivateKey.deriveSharedSecret(counterpartyPublicKey)

    // Convert the shared secret (Point) to a Uint8Array (usually x-coordinate)
    const sharedSecret: Uint8Array = new Uint8Array(
      sharedSecretPoint.x?.toArray('be', 32) ?? []
    )

    const invoiceNumberBin: number[] = Utils.toArray(
      keyDeriver.computeInvoiceNumber(protocolID, keyID),
      'utf8'
    )

    expect(specificSecret).toEqual(
      Hash.sha256hmac(Array.from(sharedSecret), invoiceNumberBin) // ✅ Convert to `number[]`
    )

    test('should throw an error for invalid protocol names', () => {
      expect(() =>
        keyDeriver.computeInvoiceNumber(protocolID, 'long' + 'a'.repeat(800))
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber(protocolID, '')
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([-3 as 0 | 1 | 2, 'otherwise valid'], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([2, 'double  space'], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([0, ''], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([0, ' a'], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([0, 'long' + 'a'.repeat(400)], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([2, 'redundant protocol protocol'], keyID)
      ).toThrow()

      expect(() =>
        keyDeriver.computeInvoiceNumber([2, 'üñî√é®sål ©0på'], keyID)
      ).toThrow()
    })
  })
})
