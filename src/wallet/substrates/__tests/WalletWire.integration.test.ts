...
describe('WalletWire Integration Tests', () => {
  ...
  describe('ProtoWallet Over Wallet Wire', () => {
    it('Throws when functions are not supported', async () => {
      ...
      ) as unknown // ✅ Explicitly cast to unknown

      async function invokeMethod<T extends unknown[]> (method: string, ...args: T): Promise<unknown> {
        if (typeof wallet === 'object' && wallet !== null && method in wallet) {
          const func = (wallet as Record<string, (...args: T) => Promise<unknown>>)[method]
          if (typeof func === 'function') {
            return await func(...args)
          }
        }
        throw new Error(`Method ${method} is not available on wallet`)
      }

      await expect(async () => await invokeMethod('createAction')).rejects.toThrow()
      await expect(async () => await invokeMethod('abortAction')).rejects.toThrow()
      await expect(async () => await invokeMethod('signAction')).rejects.toThrow()
      await expect(async () => await invokeMethod('listOutputs')).rejects.toThrow()
      await expect(async () => await invokeMethod('relinquishOutput')).rejects.toThrow()
      await expect(async () => await invokeMethod('listActions')).rejects.toThrow()
      await expect(async () => await invokeMethod('internalizeAction')).rejects.toThrow()
      await expect(async () => await invokeMethod('acquireCertificate')).rejects.toThrow()
      await expect(async () => await invokeMethod('proveCertificate')).rejects.toThrow()
      await expect(async () => await invokeMethod('listCertificates')).rejects.toThrow()
      await expect(async () => await invokeMethod('relinquishCertificate')).rejects.toThrow()
      await expect(async () => await invokeMethod('getHeight')).rejects.toThrow()
      await expect(async () => await invokeMethod('getHeaderForHeight')).rejects.toThrow()

      // TODO: Remove these two from the throw list once they are implemented.
      await expect(async () => await invokeMethod('discoverByIdentityKey')).rejects.toThrow()
      await expect(async () => await invokeMethod('discoverByAttributes')).rejects.toThrow()
    })
  })
...
it('Validates the revealSpecificKeyLinkage function', async () => {
  ...
  const computeInvoiceNumber = function (protocolID, keyID) {
    ...
    if (typeof protocolName !== 'string') {
      throw new Error('Protocol name must be a string')
    }
...
describe('encrypt and decrypt', () => {
  it('should encrypt and decrypt data correctly', async () => {
    const userKey = PrivateKey.fromRandom()
    const counterpartyKey = PrivateKey.fromRandom()
    const userWallet = createTestWalletWire(
      new CompletedProtoWallet(userKey)
    )
...
describe('getHeight and getHeaderForHeight', () => {
  ...
  it('should throw an error when getHeight fails', async () => {
    ...
    await expect(wallet.getHeight({})).rejects.toThrow(
