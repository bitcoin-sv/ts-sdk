...
} from "../../../dist/cjs/src/totp/pike-totp.js";
import HD from "../../../dist/cjs/src/compat/HD.js";

export const makeMockPKI = (xpub: string): string => {
  const contactBaseHD = HD.fromString(xpub);
...
const aliceXPrivHD = HD.fromString(aliceXPriv);
const bobXPub =
...
describe("GenerateTotpForContact", () => {
  ...
  it("should generate a TOTP proper number of digits", () => {
    expect(
      generateTotpForContact(aliceXPrivHD, mockBobContact, 60, 2).length
    ).toBe(2);
    expect(
      generateTotpForContact(aliceXPrivHD, mockBobContact, 60, 4).length
    ).toBe(4);
    expect(
      generateTotpForContact(aliceXPrivHD, mockBobContact, 60, 6).length
    ).toBe(6);
  });
  ...
  test.each([
    ...
    expect(
      generateTotpForContact(aliceXPrivHD, mockBobContact, period, 4)
    ).toEqual(expected);
...
describe("ValidateTotpForContact", () => {
  ...
  it("should validate a passcode for the given contact", () => {
    const passcode = generateTotpForContact(
      aliceXPrivHD,
      mockBobContact,
      30,
      2
