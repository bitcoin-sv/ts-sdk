  ...
  describe("Broadcast", () => {
    ...
    describe("BEEF", () => {
      it("Serialization and deserialization", async () => {
        ...
        if (!tx) throw new Error("Transaction could not be deserialized.");
        if (!tx.inputs.length) throw new Error("Transaction has no inputs.");
        if (!tx.inputs[0].sourceTransaction)
          throw new Error("Source transaction is missing.");
        if (!tx.inputs[0].sourceTransaction.merklePath)
          throw new Error("Merkle path is missing.");
