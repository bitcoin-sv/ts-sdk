...
describe('Transaction', () => {
  ...
  describe('Signing', () => {
    it('Signs unlocking script templates, hydrating the scripts', async () => {
      ...
      expect(spendTx.inputs.length).toBeGreaterThan(0) // Ensure input exists

      await spendTx.fee()
      await spendTx.sign()

      expect(spendTx.inputs.length).toBeGreaterThan(0) // Ensure input still exists
      expect(spendTx.inputs[0]?.unlockingScript).toBeDefined() // Ensure unlockingScript exists
      expect(spendTx.inputs[0]?.unlockingScript?.chunks?.length ?? 0).toBe(2)
    })
  })
...
describe('Fees', () => {
  ...
  it('Distributes change randomly among multiple change outputs', async () => {
    ...
    expect(spendTx.outputs.reduce((a, b) => a + (b.satoshis ?? 0), 0)).toEqual(
      897
    )
    ...
    it('Distributes change randomly among multiple change outputs, with one set output', async () => {
      ...
      expect(
        spendTx.outputs.reduce((a, b) => a + (b.satoshis ?? 0), 0)
      ).toEqual(8)
    })
  ...
  describe('Broadcast', () => {
    it('Broadcasts with the default Broadcaster instance', async () => {
      ...
      (global as unknown as { window: { fetch: typeof mockedFetch } }).window = { fetch: mockedFetch }
  ...
  it('Verifies the transaction from the BEEF spec with a default chain tracker', async () => {
    ...
    (global as unknown as { window: { fetch: typeof mockFetch } }).window = { fetch: mockFetch }
  ...
  describe('vectors: sighash and serialization', () => {
    sighashVectors.forEach((vector, i) => {
      ...
      it(`should pass bitcoin-abc sighash test vector ${i}`, () => {
        const txbuf = toArray(String(vector[0]), 'hex') // ✅ Ensure it's a string
        const scriptbuf = toArray(String(vector[1]), 'hex') // ✅ Ensure it's a string
        const subScript = Script.fromBinary(scriptbuf)
        const nIn = vector[2] as number
        const nHashType = vector[3] as number
        const sighashBuf = toArray(String(vector[4]), 'hex') // ✅ Ensure it's a string
        const transaction = Transaction.fromBinary(txbuf)

        // make sure transaction to/from buffer is isomorphic
        expect(toHex(transaction.toBinary())).toEqual(toHex(txbuf))
        ...
        const preimage = TransactionSignature.format({
          sourceTXID: verifyTruthy(input.sourceTXID),
          sourceOutputIndex: verifyTruthy(input.sourceOutputIndex),
          sourceSatoshis: verifyTruthy(valueBn),
          transactionVersion: tx.version,
          otherInputs,
          outputs: tx.outputs,
          inputIndex: nIn,
          subscript: verifyTruthy(subScript),
    ...
    validTransactions.forEach((vector, i) => {
      ...
      it(`should correctly serialized/deserialize tx_valid test vector ${i}`, () => {
        ...
        const expectedBin = toArray(String(vector[1]), 'hex')
