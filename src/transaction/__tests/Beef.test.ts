...
describe('Beef tests', () => {
  ...
  const chainTracker = {
    ...
    isValidRootForHeight: async (root: string, height: number) => {
      ...
      if (knownRoots[height] !== undefined) {
        return root === knownRoots[height] // ✅ Explicitly check for known height
      }
  ...
  test('0_mergeTransaction', async () => {
...

    {
      ...
      beef.bumps[0].path[0][1].hash =
        '46ebdb404ec59871c8e2b00e41d8090d28a0d8a190d44606e895dd0d013bca00'
      expect(beef.isValid(undefined)).toBe(true)
