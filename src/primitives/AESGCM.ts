...
function shiftRows (state: number[][]): void {
  ...
  for (i = 1; i < 4; i++) {
    for (j = 0; j < ((i - 1) % 2) + 1; j++) {
      ...
      // Set an explicit condition instead of `while (true)`
      let firstIteration = true
      while (firstIteration || l !== j) {
        firstIteration = false // Ensures at least one loop execution
        l = k + i
        if (l >= 4) {
          l -= 4
        }
...
function mixColumns (state: number[][]): void {
  ...
  for (i = 0; i < 4; i++) {
    temp[0][i] =
      finiteFieldMultiplication(0x02, state[0][i]) ^
      finiteFieldMultiplication(0x03, state[1][i]) ^
      state[2][i] ^
      state[3][i]
    temp[1][i] =
      state[0][i] ^
      finiteFieldMultiplication(0x02, state[1][i]) ^
      finiteFieldMultiplication(0x03, state[2][i]) ^
      state[3][i]
    temp[2][i] =
      state[0][i] ^
      state[1][i] ^
      finiteFieldMultiplication(0x02, state[2][i]) ^
      finiteFieldMultiplication(0x03, state[3][i])
    temp[3][i] =
      finiteFieldMultiplication(0x03, state[0][i]) ^
      state[1][i] ^
      state[2][i] ^
...
function keyExpansion (roundLimit: number, key: number[]): number[][] {
  ...
  for (i = 0; i < key.length; i++) {
    ...
    result[Math.floor(i / 4)].push(key[i])
  }
  ...
  for (i = nK; i < 4 * roundLimit; i++) {
    ...
    if (i % nK === 0) {
      ...
      for (j = 0; j < 4; j++) {
        temp[j] = temp[j] ^ Rcon[Math.floor(i / nK)][j]
      }
    } else if (nK > 6 && i % nK === 4) {
      subWord(temp)
    }
...
export function AES (input: number[], key: number[]): number[] {
  ...
  for (i = 0; i < 15; i++) {
    state[Math.floor(i / 4)].push(input[(i * 4) % 15])
  }
...
const R: number[] = [0xe1, ...createZeroBlock(15)]

export const exclusiveOR = function (
  block0: number[],
  block1: number[]
): number[] {
  let i
...
export function ghash (input: number[], hashSubKey: number[]): number[] {
  ...
  for (i = 0; i < input.length; i += 16) {
    result = multiply(
      exclusiveOR(result, input.slice(i, Math.min(i + 16, input.length))),
      hashSubKey
    )
  }
...
export function AESGCM (
  ...
  if (initializationVector.length === 12) {
    ...
    preCounterBlock = ghash(
      preCounterBlock
        ...
        .concat(getBytes(initializationVector.length * 8)),
      hashSubKey
    )
  }
  ...
  if (additionalAuthenticatedData.length === 0) {
    ...
    plainTag = plainTag.concat(
      createZeroBlock(16 - (additionalAuthenticatedData.length % 16))
    )
  }
...
export function AESGCMDecrypt (
  ...
  if (cipherText.length === 0) {
    ...
    compareTag = compareTag.concat(
      createZeroBlock(16 - (cipherText.length % 16))
    )
  }
  ...
  const calculatedTag = gctr(
    ghash(compareTag, hashSubKey),
    preCounterBlock,
    key
  )
