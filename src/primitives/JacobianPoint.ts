...
export default class JacobianPoint extends BasePoint {
  ...
  eqXToP(x: BigNumber): boolean {
    ...
    const rx = x.toRed(this.curve?.red!).redMul(zs); // ✅ Use optional chaining (`?.`)
    ...
    const xc = x.clone();
    const t = this.curve?.redN!.redMul(zs); // ✅ Ensure `redN` exists

    while (true) {
      xc.iadd(this.curve?.n!); // ✅ Non-null assertion since `n` always exists
